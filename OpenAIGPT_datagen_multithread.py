...
def OpenAIGPT_datagen(args):
    ...
    with jsonlines.open(output_path, "a" if os.path.exists(output_path) else "w") as writer:
        with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
            ...
            for future in tqdm(futures, total=len(items_to_process), desc="Processing items"):
                flag, item = future.result()
                if flag:
                    writer.write(item)
                else:
                    print('Item：' + str(item['id']) + '|请求失败|Error:' + item['error'])
                    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default="gpt-3.5-turbo", help="Name of the OpenAIGPT model to use.")
    parser.add_argument("--keys_path", type=str, required=True, help="API key for the OpenAIGPT service.")
    ...
    parser.add_argument("--max_workers", type=int, default=10, help="Maximum number of workers for concurrent processing.")
